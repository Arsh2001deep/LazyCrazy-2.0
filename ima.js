...
export const newRowsima = (cyr, byr, gyr, m, d, gm, gd) => {
    ...
    data.map(({ year, attempt, grad, timeslot, session }) => {
        if (count == 0) {
            document.getElementById('main').innerHTML += `
                ...
                <th>ATTEMPTS</th>
                <th>AGE LIMIT</th>
                <th>SESSION START AT IMA/INA</th>
...
const attemptsChecker = (cyr, byr, gyr, m, d, gm, gd) => {
    ...
    let session = []
    session[0] = `01-01-${cyr+1}`//mmddyy
    session[1] = `07-01-${cyr+1}`
    ...
    for (let i = cyr; i <= byr + 25; i++) {
        ...
        timeslot[2] = `${cyr+1}-01-01`
        timeslot[3] = `${cyr+1}-07-01`
        ...
        data.push({
            ...
            session: [`01-01-${cyr + 1}`, `01-07-${cyr+1}`]
        })
